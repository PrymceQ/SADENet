...
Global Attention Upsample Module
    ...
    y = torch.randn(1,128,4,4)
    in_ch, out_ch = 64, 128
    output.shape = (1, 128, 8, 8)
'''
class My_GAU(nn.Module): 
	def __init__(self,in_ch,out_ch):
		super(My_GAU, self).__init__()
        # attention
		...
		self.conv2 = ConvModule(in_ch, out_ch, kernel_size=3, stride=1, padding=1, norm_cfg = dict(type='BN2d'),act_cfg=dict(type='ReLU')) # Batchnorm的batch_size不能为1
...
class Focus(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=0, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = ConvModule(in_channels=c1 * 4, out_channels=c2, kernel_size=k, stride=s, padding=p, groups=g)      # 这里输入通道变成了4倍

    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        return self.conv(torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))

'''
Global Attention Downsample Module  no_use
    x = torch.randn(1,64,8,8)
    y = torch.randn(1,128,4,4)
    in_ch, out_ch = 64, 128
    output.shape = (1, 128, 4, 4)
'''
class My_GAD(nn.Module):
	def __init__(self,in_ch,out_ch):
		super(My_GAD, self).__init__()
        # attention
		self.conv1 = nn.Sequential(
            ConvModule(1, 1, kernel_size=1, stride=1, padding=0, norm_cfg = dict(type='BN2d')),
			nn.Sigmoid()
		)
		self.conv2 = ConvModule(in_ch, out_ch, kernel_size=3, stride=1, padding=1, norm_cfg = dict(type='BN2d'),act_cfg=dict(type='ReLU')) # Batchnorm的batch_size不能为1
		self.conv3 = Focus(out_ch, out_ch)
		self.conv4 = ConvModule(out_ch, out_ch, kernel_size=3, stride=2, padding=1)
	...
	def forward(self,x,y):
		...
		x_down = self.conv3(x)
		x = self.conv4(x)
		x = self.conv1(torch.mean(x,dim=1,keepdim=True))
		z = torch.mul(x, y)
...
class CAM(nn.Module):
    def __init__(self, inplanes, reduction_ratio=1, fpn_lvl=4): #inplanes=256
        ...
        self.dila_conv = nn.Sequential(nn.Conv2d(inplanes * fpn_lvl // reduction_ratio, inplanes // reduction_ratio, # in=4*inplanes, out=inplanes,size_no_change
    ...
    def forward(self, x):
        ...
        lvl_fea = self.dila_conv(multi_feats) # out_c=256
...
class MySSFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ch_all = 256, stride = [4, 8, 16, 32]

        # build attention map
        att_list = self.CAM(laterals)
        
        laterals = [(1 + att_list[i]) * laterals[i] for i in range(len(laterals))]  # ch_all = 256, stride = [4, 8, 16, 32]
