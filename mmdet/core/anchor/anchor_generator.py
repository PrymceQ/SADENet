...
# MarbleAnchorGenerator
@PRIOR_GENERATORS.register_module()
class MarbleAnchorGenerator(AnchorGenerator):
    def __init__(self,
                 strides,
                 ratios,
                 scales=None,
        ...
        self.centers = centers
        self.center_offset = center_offset
        self.marble_offset = marble_offset   # NewAdd
        self.base_anchors = self.gen_base_anchors()
        ...
        for i, base_size in enumerate(self.base_sizes):
            ...
            multi_level_base_anchors.append(
                self.gen_single_level_base_anchors(
                    ...
                    ratios=self.ratios,
                    center=center,
                    marble_offset = self.marble_offset # NewAdd
                    ))
        return multi_level_base_anchors
    ...
    def gen_single_level_base_anchors(self,
        ...
        # newadd
        marble_anchors = [
            - w/2 - 0.5 * ws, - h/2 - 0.5 * hs, - w/2 + 0.5 * ws,
            - h/2 + 0.5 * hs
        ]
        marble_anchors = torch.stack(marble_anchors, dim=-1)
        
        base_anchors = torch.cat((base_anchors, marble_anchors),0)
